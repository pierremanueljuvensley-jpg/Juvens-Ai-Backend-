...
# ---- CORS FIX ----
# Explicitly allow the current Cloudflare Pages frontend.
# The regex also permits other *.pages.dev preview/deployment URLs.
origins = [
    "https://aijuvensley1.pages.dev",
]
...
def cors_test():
    return {
        "status": "ok",
        "cors": "configured",
        "frontend": "https://aijuvensley1.pages.dev"
    }
...
def register(data: RegisterIn):
    ...
    try:
        ...
        logger.exception("Erreur interne pendant /auth/register")
        raise HTTPException(500, "Erreur interne lors de la création du compte")
    finally:
        con.close()
...
def add_transaction(data: TransactionIn, user=Depends(current_user)):
    ...
    cur = con.execute(
        ...
        (
            user["id"], data.date, data.description, data.debit_account,
            data.credit_account, data.amount, data.reference
        )
    )
    con.commit()
    con.close()
...
def add_invoice(data: InvoiceIn, user=Depends(current_user)):
    ...
    cur = con.execute(
        ...
        (
            user["id"], data.number, data.customer, data.amount,
            data.status, data.due_date, now()
        )
    )
    con.commit()
    con.close()
...
def add_expense(data: ExpenseIn, user=Depends(current_user)):
    ...
    cur = con.execute(
        ...
        (
            user["id"], data.date, data.category,
...
def export_csv(user=Depends(current_user)):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition":
            "attachment; filename=juvens-ai-comptabilite.csv"
        }
    )
