...
# Gemini via couche de compatibilité OpenAI — 100% GRATUIT
client = OpenAI(
...
app = FastAPI(title="Juvens AI API", version="3.0.0")
...
@app.get("/")
def root():
    return {"name": "Juvens AI", "status": "ok", "version": "3.0.0"}
...
def register(data: RegisterIn):
    ...
    try:
        ...
        cur = con.execute(
            ...
            (data.email.lower(), data.name, pwd_ctx.hash(data.password), now(), is_admin)
        )
        con.commit()
...
def login(data: LoginIn):
    con = db()
    user = con.execute("SELECT * FROM users WHERE email=?", (data.email.lower(),)).fetchone()
    con.close()
...
def rename_conversation(cid: int, data: TitleIn, user=Depends(current_user)):
    con = db()
    cur = con.execute(
...
def chat(request: Request, data: ChatIn, user=Depends(current_user)):
    if not client:
        raise HTTPException(503, "GEMINI_API_KEY non configurée. Veuillez contacter l'administrateur.")
    ...
    # Récupérer l'historique (40 derniers messages)
    history = con.execute(
    ...
    system_prompt = f"""Tu es Juvens AI, un assistant intelligent et professionnel créé par Juvens.
Tu réponds dans la langue de l'utilisateur : français, créole haïtien, ou anglais.
Sois clair, précis, honnête et vraiment utile.
Quand une information est incertaine, dis-le clairement.
Pour les questions liées aux affaires, à la comptabilité et au commerce en Haïti, adapte tes réponses au contexte haïtien.
Utilisateur actuel : {user['name']}"""
    ...
    try:
        ...
        raise HTTPException(502, f"Erreur du service IA : {str(e)}")
...
async def upload_file(request: Request, file: UploadFile = File(...), user=Depends(current_user)):
    ...
    safe_name = f"user{user['id']}_{file.filename}"
    ...
    cur = con.execute(
        ...
        (user["id"], file.filename, file_path, now())
    )
    con.commit()
    con.close()
    return {"id": cur.lastrowid, "name": file.filename, "ok": True}
...
def accounting_summary(user=Depends(current_user)):
    con = db()
    revenue   = con.execute("SELECT COALESCE(SUM(amount),0) x FROM invoices WHERE user_id=? AND status='paid'", (user["id"],)).fetchone()["x"]
    expenses  = con.execute("SELECT COALESCE(SUM(amount),0) x FROM expenses WHERE user_id=?", (user["id"],)).fetchone()["x"]
    receivable = con.execute("SELECT COALESCE(SUM(amount),0) x FROM invoices WHERE user_id=? AND status='unpaid'", (user["id"],)).fetchone()["x"]
    con.close()
    return {
        "revenue": revenue,
        "expenses": expenses,
...
def add_transaction(data: TransactionIn, user=Depends(current_user)):
    ...
    cur = con.execute(
        ...
        (user["id"], data.date, data.description, data.debit_account, data.credit_account, data.amount, data.reference)
    )
    con.commit()
    con.close()
...
def add_invoice(data: InvoiceIn, user=Depends(current_user)):
    ...
    cur = con.execute(
        ...
        (user["id"], data.number, data.customer, data.amount, data.status, data.due_date, now())
    )
    con.commit()
    con.close()
    return {"id": cur.lastrowid, "ok": True}
...
def add_expense(data: ExpenseIn, user=Depends(current_user)):
    ...
    cur = con.execute(
        ...
        (user["id"], data.date, data.category, data.description, data.amount)
    )
    con.commit()
    con.close()
    return {"id": cur.lastrowid, "ok": True}
...
def export_csv(user=Depends(current_user)):
    con = db()
    rows = con.execute(
        "SELECT date,description,debit_account,credit_account,amount,reference FROM transactions WHERE user_id=? ORDER BY date,id",
        (user["id"],)
    ).fetchall()
    con.close()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Date", "Description", "Débit", "Crédit", "Montant (HTG)", "Référence"])
    for r in rows:
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=juvens-ai-comptabilite.csv"}
    )
...
def admin_stats(user=Depends(current_user)):
    ...
    stats = {
        ...
        "conversations": con.execute("SELECT COUNT(*) x FROM conversations").fetchone()["x"],
        "messages":      con.execute("SELECT COUNT(*) x FROM messages").fetchone()["x"],
        "files":         con.execute("SELECT COUNT(*) x FROM files").fetchone()["x"],
    }
    con.close()
    return stats
