...
SYSTEM_PROMPT = """
Tu es Juvens AI, un assistant IA créé par Pierre Manuel Juvensley.
Si on te demande qui t'a créé, qui est ton créateur ou ton fondateur, réponds clairement : « J'ai été créé par Pierre Manuel Juvensley. »
...
def chat(
    ...
    # Build multimodal attachment content for supported files.
    # Images are sent as data URLs; text-like files are decoded and injected as text.
    attachment_parts = []
    attachment_notes = []
    if data.attachments:
        if user_id is None:
            raise HTTPException(status_code=401, detail="Connectez-vous pour utiliser des fichiers ou des images.")
        conn = get_db()
        for item in data.attachments[:6]:
            try:
                fid = int(item.get("id"))
            except (TypeError, ValueError):
                continue
            row = conn.execute(
                "SELECT id, filename, content_type, content FROM files WHERE id = ? AND user_id = ?",
                (fid, user_id),
            ).fetchone()
            if not row:
                continue
            filename = row["filename"] or "fichier"
            ctype = row["content_type"] or "application/octet-stream"
            raw = row["content"] or b""
            if ctype.startswith("image/") and raw:
                import base64
                ...
                attachment_notes.append(f"Image jointe : {filename}")
            elif ctype.startswith("text/") or ctype in {"application/json", "text/csv"}:
                try:
                    txt = raw.decode("utf-8", errors="replace")[:30000]
                    attachment_parts.append({"type":"text", "text":f"\nContenu du fichier {filename}:\n{txt}"})
                    attachment_notes.append(f"Fichier texte : {filename}")
                except Exception:
                    attachment_notes.append(f"Fichier joint : {filename}")
            else:
                attachment_notes.append(f"Fichier joint : {filename} ({ctype})")
        conn.close()

    # IMPORTANT: plain string content for ordinary text-only requests.
    messages = [
        {
            "role": "system",
...
async def upload_file(
    ...
    cur = conn.execute(
        """
        INSERT INTO files
        (user_id, filename, content_type, size, content, created_at)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            user_id,
